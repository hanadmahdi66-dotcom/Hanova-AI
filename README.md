🤖 hanova-AI

A Local Open Source AI Application powered by hanOS — Built with React.js.

"License" (https://img.shields.io/badge/license-MIT-green.svg)
"Open Source" (https://img.shields.io/badge/Open%20Source-Yes-blue.svg)
"Built with" (https://img.shields.io/badge/Built%20with-React.js-61DAFB.svg)
"Status" (https://img.shields.io/badge/status-active-success.svg)
"Contributions" (https://img.shields.io/badge/PRs-welcome-orange.svg)

---

📖 About

hanova-AI waa Local Open Source AI Application oo ku dhisan React.js.

Mashruucan waxaa loogu talagalay in uu dadka siiyo AI experience fudud, la habeyn karo, oo bulshada ay horumarin karto.

hanova-AI wuxuu ku shaqeeyaa fikradda hanOS, oo ah deegaanka iyo ecosystem-ka ay ku dhisan yihiin AI applications-ka Hanath.

---

🛠️ Technology Stack

Frontend

- ⚛️ React.js
- 🎨 Modern UI Components
- 🚀 Fast Development Environment

AI Layer

- 🧠 AI Models Integration
- 🔍 Intelligent Processing
- 💾 Local-first architecture

---

✨ Features

- 🤖 AI-powered application
- ⚛️ Built with React.js
- 🔓 Open Source
- 🌍 Community-driven
- ⚡ Fast and lightweight
- 🛠️ Easy to customize

---

🤝 Resource Partner

hanova-AI supports collaboration with useful tools and resources that help open-source developers.

BizBot

BizBot is an AI-powered Business Lead Generation and Sales Automation platform.

🌐 Website: http://bizbot.zvo.cn

Through its resource exchange program, BizBot provides free licenses to eligible open-source developers. Users are responsible for complying with local laws, platform policies, privacy requirements, and terms of use when using third-party services.

We appreciate their support for the open-source community.
